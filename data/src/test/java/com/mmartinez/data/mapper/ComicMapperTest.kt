...
class ComicMapperTest {
    private lateinit var cut: ComicListMapper

    companion object {
        private val DATA_MODEL = Fixtures.ResponseData.Data.model
        private val DOMAIN_MODEL = Fixtures.ComicListDomainModel.Data.model
    }
    ...
    fun ` data model is correctly mapped to domain model`() {
        ...
        domainModel.run {
            ...
            assertEquals(item.title, DOMAIN_MODEL.list[0].title)
            assertEquals(item.imageUrl, DOMAIN_MODEL.list[0].imageUrl)
            assertEquals(item.variantDescription, DOMAIN_MODEL.list[0].variantDescription)
        }
    }
